...
async function startServer() {
  const app = express();
  // In Google Cloud Run containers, Cloud Run injects K_SERVICE and PORT (default 8080).
  // In AI Studio workspace, port 3000 is strictly required by the reverse proxy.
  const PORT = process.env.K_SERVICE ? (Number(process.env.PORT) || 8080) : 3000;
